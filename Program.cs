...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            int most = shows.Max(s => s.EpisodeCount);
            return shows.FirstOrDefault(s => s.EpisodeCount == most);
        }
        ...
        {
            int ending = shows.Where(s => s.EndYear >= 2000).Min(s => s.EndYear);
            return shows.OrderBy(s => s.EndYear).FirstOrDefault(s => s.EndYear == ending);
        }
